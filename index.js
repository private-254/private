...
async function saveSessionFromConfig() {
  try {
    ...
    if (!config.SESSION_ID.includes('DAVE-MD:~')) return false;

    const base64Data = config.SESSION_ID.split("DAVE-MD:~")[1];
...
async function tylor() {
  try {
    ...
    if (config.SESSION_ID && config.SESSION_ID.includes("DAVE-MD:~")) {
