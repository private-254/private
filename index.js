...
> *Report any error to the dev*`;
  ...
  malvin.ev.on('messages.upsert', async(mek) => {
    ...
    const newsletterJids = ["120363401297349965@newsletter"];
    const emojis = ["😂", "🥺", "👍", "☺️", "🥹", "♥️", "🩵"];
    ...
    events.commands.map(async(command) => {
      if (body && command.on === "body") {
        command.function(malvin, mek, m,{from, quoted, body, isCmd, command, args, q, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply});
