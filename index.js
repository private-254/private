...
function ensureEnv(envPath) {
  try {
    ...
    if (!fsSync.existsSync(envPath)) {
      ...
      console.log(chalk.yellow("Set SESSION_ID to DAVE-X~<base64 json creds> for seamless login."));
      return;
    }
...
async function loadSession() {
  ...
  // Ask for SESSION_ID if not provided
...
async function connectToWA() {
  ...
  malvin.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(chalk.green("DAVE-X Connected ✅"));
      ...
      try {
        ...
        const ownername = "Dave Tech";
        const prefix = getPrefix();
        const username = "Digladoo";
        const mrmalvin = `https://t.me/${username}`;
        const repoUrl = "https://github.com/DaveTech";
...
Telegram: ${mrmalvin}`;
    ...
    try {
      ...
      for (const call of calls) {
        ...
        await malvin.sendMessage(from, {
          text: config.REJECT_MSG || '*Busy call later*'
        });
  ...
  malvin.ev.on('messages.upsert', async(mek) => {
    ...
    // FIXED: Should NOT ignore owner commands
    const isCreator = creatorJids.includes(sender) || isMe;

    // Terminal command for owners only
  ...
  malvin.getName = (jid, withoutContact = false) => {
    ...
    if (id.endsWith('@g.us')) {
      ...
      v = id === '0@s.whatsapp.net' ? { id, name: 'WhatsApp' } : id === malvin.decodeJid(malvin.user.id) ? malvin.user : store.contacts[id] || {};
      return (
